...
const CreateExpenseForm = () => {
	...
	const handleSelectChange = useCallback(
		...
		[memberData, retrievedUserData.friend]
	);

	return (
		<form onSubmit={handleSubmit(onSubmit)} className='flex flex--column width--90-per m--0-auto'>
			<div className='mt--30 flex flex--column'>
				<label htmlFor='billName' className='font-family--medium font-size--18 mb--10'>
					Bill Name:
				</label>
				<input
					type='text'
					placeholder='Eg: Friend birthday'
					{...register('billName', { required: true, maxLength: 80 })}
					className='height--50-px b-radius--10 b--none pl--15'
				/>
				{errors.billName && <span className='error font-size--14px mt--10'>Bill Name is required*</span>}
			</div>

			<div className='mt--30 flex flex--column'>
				<label htmlFor='typeOfBill' className='font-family--medium font-size--18 mb--10'>
					Type of bill:
				</label>
				<input
					type='text'
					placeholder='Eg: fast food or gift'
					{...register('typeOfBill', { required: true, maxLength: 100 })}
					className='height--50-px b-radius--10 b--none pl--15'
				/>
				{errors.typeOfBill && <span className='error font-size--14px mt--10'>Type of bill Name is required*</span>}
			</div>

			<div className='mt--30 flex flex--column'>
				<label htmlFor='amount' className='font-family--medium font-size--18 mb--10'>
					Amount:
				</label>
				<input
					type='number'
				...
				/>
				{errors.amount && <span className='error font-size--14px mt--10'>Bill amount is required*</span>}
			</div>

			<div className='mt--30 mb--30 flex flex--column'>
				<label htmlFor='paidBy' className='font-family--medium font-size--18 mb--10'>
					Paid by:
				</label>
				<select
				...
				{errors.paidBy && <span className='error font-size--14px mt--10'>Paid by is required*</span>}
			</div>
			...
			<div className='mb--30 flex flex--column'>
						...
						return (
							<Fragment key={index}>
								<label htmlFor='chooseCb' className='cursor--pointer'>
									<input type='checkbox' value={value} id={name} {...register('chooseCb', { validate: validateCheckbox })} />
				...
				{errors.chooseCb && <span className='error font-size--14px mt--10'>Please select at least one checkbox*</span>}
			</div>

			<div className='mt--30 mb--30 flex flex--column'>
				<input
					type='submit'
					className='primary--btn font-size--16px font-weight--600 line-height--20px b-radius--25 width--100-px cursor--pointer m--0-auto'
				/>
			</div>
		</form>
	);
