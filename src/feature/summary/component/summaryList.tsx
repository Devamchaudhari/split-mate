...
const SummaryList = () => {
	...
	const handleOnStatus = useCallback(
		(billIndex: number, status: string) => {
	...
	const handleOnMemberClick = useCallback(
		(expenseMemberIndex: number, memberIndex: number) => {
	...
	return (
		<div className='ml--30 pb--20 mr--30 flex  flex--wrap box-size--border-box'>
			{!isEmpty(expenseData) &&
				expenseData.map((expenseData: IExpenseData, index: number) => {
					...
					return (
						<div key={index} className={`mr--30 mt--30 ${summaryStyle['summary__list--box']}`}>
							<div className={`ml--20 flex justify__content--between mr--20 flex--wrap ${summaryStyle['summary__list--content']}`}>
								<p className='font-family--bold font-size--28 text--capitalize flex flex--column line-height--33px'>
									<span className='font-size--16px mb--10 font-family--semi-bold'>Bill name:</span>
									{billName}
								</p>

								<p className='font-family--bold font-size--28 text--capitalize flex flex--column'>
									<span className='font-size--16px mb--10 font-family--semi-bold'>Created Date:</span>
									{createdDate}
								</p>

								<p className='font-family--bold font-size--28 text--capitalize flex flex--column line-height--33px mb--25'>
									<span className='font-size--16px font-family--semi-bold'>Bill description:</span>
									{typeOfBill}
								</p>

								<p className='font-family--bold font-size--28 text--capitalize flex flex--column'>
									<span className='font-size--16px mb--10 font-family--semi-bold'>
										{paidBy === 'Olivia Davis' ? ' Paid By you:' : 'Paid By:'}{' '}
									</span>{' '}
									{paidBy}
								</p>

								<p className='font-family--bold font-size--28 text--capitalize flex flex--column text--blue'>
									{' '}
									<span className='font-size--16px mb--10 font-family--semi-bold text--black'>Total Bill: </span> &#8377;&nbsp;
									{amount}
								</p>
							</div>
							<div className={`${summaryStyle['summary__member--wrapper']} flex mb--30 flex--wrap`}>
								{chooseCb?.map((member: string, memberIndex: number) => {
									return (
										<div
											className={`flex align__items--center ml--15 ${summaryStyle['summary__member--content']}`}
										...
										>
											...
											<p className='font-family--semi-bold font-size--18 flex flex--column'>
												{`${memberMapper[member].name}`}
												<span className='font-size--14px mt--10 font-family--semi-bold'>
													Amount:&nbsp;
													{amountStatus || Object.values(friendBilStatus)[memberIndex] ? (
														'Paid'
													) : (
														<Fragment>
															&#8377;
							...
							<div
								className={`${summaryStyle['summary__list-btn']} flex width--full justify__content--evenly mb--20 mt--30 flex--wrap `}
							>
								{!amountStatus && (
									<button
										className='success--btn font-size--16px font-weight--600 line-height--20px b-radius--25 width--auto cursor--pointer'
								...
								{amountStatus && (
									<button
										className='primary--btn font-size--16px font-weight--600 line-height--20px b-radius--25 width--auto cursor--pointer'
								...
								<button
									className='error-btn font-size--16px font-weight--600 line-height--20px b-radius--25 width--auto cursor--pointer'
			...
			{isEmpty(expenseListData) && (
				<p className='flex justify__content--center font-size--24 font-family--bold width--full mt--40'>No Expense added💲</p>
			)}
		</div>
	);
};
